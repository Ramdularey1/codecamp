    ...
    <div className="app-shell">
      <div className="page-header sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <h1 className="page-title mt-2">Contest Leaderboard</h1>
          <p className="page-subtitle mt-2">Live rankings update as contest submissions are scored.</p>
        </div>
      ...
      {isLoading ? (
        <div className="panel p-8 text-center text-slate-400">Loading contest leaderboard...</div>
      ) : data.length === 0 ? (
        <div className="panel p-8 text-center text-slate-400">No contest scores yet.</div>
      ) : (
        <>
            ...
            {topThree.map((item, index) => (
              ...
              >
                ...
                <p className="mt-2 text-sm text-slate-400">
                  {index === 0 ? "Leading this contest" : "Contest contender"}
                </p>
              </div>
            ))}
          </div>
              ...
              {data.map((item, index) => (
                <tr key={item.user?._id || item.user?.username || index} className="transition hover:bg-white/[0.03]">
                  ...
                  <td>
                    ...
                    <div className="text-xs text-slate-500">Contest participant</div>
                  </td>
